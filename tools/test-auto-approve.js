...
//     Frost shows a login page only when somebody is there to see it, so real
//     idle time would make these tests depend on whether anyone happened to
//     touch the keyboard: green in CI on a fresh runner, red on a desktop five
//     minutes after you started them and walked away.
//
...
/** The app's own idea of "nobody is here", so these cannot drift apart. */
const { AWAY_IDLE_SEC } = await import("../dist/schedule.js");
...
/**
 * Whoever is at the machine, as the scenarios decide. Frost reads this through
 * `powerMonitor.getSystemIdleTime()` whenever it matters — a login started with
 * nobody there can end with the user watching — so one value covers a whole run
 * and can change part-way through it.
 */
let idleSec = 0;
...
function beAway() {
    idleSec = AWAY_IDLE_SEC * 2;
}

function controlPresence() {
    powerMonitor.getSystemIdleTime = () => idleSec;
    return powerMonitor.getSystemIdleTime() === idleSec;
}
...
/**
 * Nobody is at the machine. A window shown now, or a tab opened in a browser
 * nobody is looking at, is a login page that expires unseen — so the attempt is
 * held instead, hidden and still being driven, and handed over the moment
 * somebody turns up.
 */
async function unattendedLoginIsParked(frost) {
    newRun("a refresh that needs the user, with nobody at the machine", {
        idp: "signin",
    });
    beAway();
    ...
    await waitFor(
        () => run.trail.includes(`page:${IDP}/signin`),
        "the sign-in page to be reached"
    );
    // Long enough that a hand-over, if one were coming, would have happened.
    await new Promise((resolve) => setTimeout(resolve, 2000));

    check(!run.everVisible, "a login window was shown to an empty room");
    check(run.notifications.length === 0, "an empty room was notified");

    // The user turns up; the page they get is the one Frost has been holding.
    beHere();
    const window = await waitFor(
        visibleWindow,
        "the held login to be shown once the user is back"
    );
    await signInOnPage(window);
    await withTimeout(refreshing, "the refresh to finish");
...
const TESTS = [
    ...
    [
        "nobody there: login held hidden, shown when the user comes back",
        unattendedLoginIsParked,
        {},
    ],
];
...
async function main() {
    ...
    assert.ok(
        controlPresence(),
        "could not control the idle time; the tests would depend on whether anyone touched the keyboard"
    );
    assert.ok(
        recordNotifications(),
